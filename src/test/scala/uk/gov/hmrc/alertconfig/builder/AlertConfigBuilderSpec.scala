  ...
  "AlertConfigBuilder" should {
    ...
    "throw exception and stop processing when app config directory not found" in {
      ...
      intercept[FileNotFoundException] {
        val fakeConfig = new AlertConfig {
          override def alertConfig: Seq[AlertConfigBuilder] = Seq(AlertConfigBuilder("service1", integrations = Seq("h1", "h2")))

          override def environmentConfig: Seq[EnvironmentAlertBuilder] = envConfig
        }

        AlertsYamlBuilder.convert(Seq(fakeConfig), Environment.Production)
      }
    }
//
